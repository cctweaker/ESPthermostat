...
void init_webserver()
{
    ...
    server.on("/thermostat", cfg_page_thermostat);
...
void page_main()
{
    ...
    if (onewire_device_count > 0)
    {
        ...
        for (uint8_t i = 0; i < onewire_device_count; i++)
        {
            sprintf_P(buffer, td_temp_c, i + 1, onewire_temp[i]);
            server.sendContent(buffer);
    ...
    {
        server.sendContent("No temperature sensors detected<br>");
    }
    ...
    sprintf_P(buffer, menu_entry, "/thermostat", "", menu_entry_thermostat);
    server.sendContent(buffer);

    sprintf_P(buffer, menu_entry, "/update", menu_entry_check_update, menu_entry_update);
    server.sendContent(buffer);
    sprintf_P(buffer, menu_entry, "/format", menu_entry_check_erase, menu_entry_erase);
    server.sendContent(buffer);
    sprintf_P(buffer, menu_entry, "/restart", menu_entry_check_restart, menu_entry_restart);
    server.sendContent(buffer);
...
void cfg_page_mqtt()
{
    ...
    if (server.hasArg("TELE"))
        strlcpy(TELE, server.arg("TELE").c_str(), sizeof(TELE));
    ...
    server.sendContent(buffer);
    sprintf_P(buffer, form_text_field_string, txt_mqtt_tele, "TELE", TELE);
    server.sendContent(buffer);
...
void cfg_page_thermostat()
{
    ...
    {
        server.sendContent_P(save_to_config);
        server.sendContent(fs_save_thermostat());
    }
